...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(embed_size, embed_size, kernel_size=2, stride=1, padding='same')
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        states = torch.cat([*last_hidden], dim=1), torch.cat([*last_cell], dim=1)
        init_decoder_hidden = self.h_projection(states[0])
        init_decoder_cell = self.c_projection(states[1])
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):  # the shape of y_t is (1, b, e)
            Y_t = torch.squeeze(Y_t, 0)
            Ybar_t = torch.cat([Y_t, o_prev], dim=-1)
            dec_states, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens).squeeze(1)  # (b, 2h)
